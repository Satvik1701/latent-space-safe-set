...
def generate_novice_demos(env, env_name, agent, model_dir):
    ...
    for checkpoint in checkpoints:
        ...
        for i in range(episode_count):
            ...
            learning_returns.append(acc_reward) #accumulated reward over episode
            learning_rewards.append(gt_rewards) #ground truth reward for each step in trajectory

    return demonstrations, learning_returns, learning_rewards


def create_training_data(demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_length): #demonstrations is ranked list t1 > t2 > t3 ...
    ...
    for n in range(num_trajs):
        ...
        training_labels.append(label) #create aritifical uniform dist over {0,1}
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 32, 4, stride=2) #64x64x3
        self.conv2 = nn.Conv2d(32, 64, 4, stride=3)
        self.conv3 = nn.Conv2d(64, 128, 4, stride=2)
        self.conv4 = nn.Conv2d(128, 256, 4, stride=2)
        self.fc1 = nn.Linear(256, 32) #32 size of latent space, retraining with 8
    ...
    def cum_return(self, traj):
        ...
        x = F.leaky_relu(self.conv1(x)) #maybe change to relu
...
if __name__=="__main__":
    ...
    parser.add_argument('--reward_model_path', default='./trex_ens', help="name and location for learned model params, e.g. ./learned_models/breakout.params")
    ...
    # demonstrations = [x for _, x in sorted(zip(learning_returns,demonstrations), key=lambda pair: pair[0])] #accumulated reward over episode
    ...
    #----------Demos for Pointbot check-------------------
    fil = open("demo_data.pkl",'rb')
    object_file = pickle.load(fil)
    fil.close()
    all_traj = np.array(object_file['images'])
    pairs_pref = np.array(object_file['prefs'])
    demonstrations = [all_traj[i] for i in range(all_traj.shape[0])]
    pairs = 500
    training_obs = []
    training_labels = []
    for i in range(pairs):
        bad, good = pairs_pref[i]
        # 2nd is preferred over the 1st
        good_idx = np.random.randint(0, 2)
        if good_idx == 1:
            training_obs.append((all_traj[bad], all_traj[good]))
        else:
            training_obs.append((all_traj[good], all_traj[bad]))
        training_labels.append(good_idx)
    
    # print(training_obs, training_labels)
    # raise Exception
    # Now we create a reward network and optimize it using the training data.
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    total_demos = [100]
    sampling_with_replacement = [30]
    num_ens = 5
    ...
    for a, b in arr:
        exp_training_labels, exp_training_obs = training_labels[:b], training_obs[:b]
        for i in range(num_ens):
            ...
            idxs = np.random.randint(0, b, a)
            print(idxs)
            member_training_labels = [exp_training_labels[i] for i in idxs]
            member_trianing_obs = [exp_training_obs[i] for i in idxs]
            optimizer = optim.Adam(reward_net.parameters(),  lr=lr, weight_decay=weight_decay)
            pat = args.reward_model_path + str(a) + "_" + str(b) + "_" + str(i) + ".pth"
            print(pat)
            learn_reward(reward_net, optimizer, member_trianing_obs, member_training_labels, num_iter, l1_reg, pat)
            #save reward network
            torch.save(reward_net.state_dict(), pat)
